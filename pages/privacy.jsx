/* global React */
const { Reveal, Mono, Eyebrow, Btn, PageShell, ChapterOpener, IMG } = window;
...
const Page = () => (
  <PageShell activePage="privacy">
    <ChapterOpener
      ...
      imageHue={5}
      imageLabel="PRIVACY"
      breadcrumbs={[{ label: "BIMC CosMedic", href: "index.html" }, { label: "Privacy & Terms" }]}
    />

    {/* Updated banner */}
    <section style={{ background: "var(--paper-warm)", borderBottom: "1px solid var(--ink-20)", padding: "28px var(--page-x)" }}>
            ...
            <Reveal key={s.id} delay={Math.min(i, 6) * 40}>
              <article id={s.id} className="privacy-section">
                ...
                {s.body && s.body.map((p, j) => (
                  <p key={j} className="privacy-p">{p}</p>
                ))}
                {s.list && (
      ...
      <Reveal>
        <div className="privacy-contact">
          ...
          <div className="privacy-contact-meta">
            <div>
              <Mono>Email</Mono>
              <a href="mailto:privacy@bimcbali.com" className="privacy-contact-link">privacy@bimcbali.com</a>
            </div>
            <div>
              <Mono>Post</Mono>
              <span className="privacy-contact-addr">
                Data Protection Officer<br />
                BIMC CosMedic, Jl. Bypass Ngurah Rai 100X<br />
                Kuta, Bali 80361, Indonesia
              </span>
            </div>
            <div style={{ marginTop: 12 }}>
              <Btn kind="ghost" as="a" href="contact.html">General contact</Btn>
            </div>
          </div>
        </div>
      </Reveal>
