...
// Format AUD integer → "Rp X.XXX.XXX" (rounded to nearest 50,000 IDR)
const fmtIDR = (aud) => "Rp " + (Math.round(aud * 10500 / 50000) * 50000).toLocaleString("de-DE");

const Page = () => {
  const subData = window.SUBCATEGORY_DATA || {};
  const byDiscipline = window.SUBCATEGORIES_BY_DISCIPLINE || {};
  ...
  return (
    <PageShell activePage="pricing">
      <ChapterOpener
        ...
        imageHue={2}
        imageLabel="PRICING"
        breadcrumbs={[{ label: "BIMC CosMedic", href: "index.html" }, { label: "Pricing" }]}
      />

      <section className="page-section">
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          {TREATMENT_LIST.map((discipline, dIdx) => {
            const subs = byDiscipline[discipline.slug] || [];
            if (subs.length === 0) return null;

            return (
              <div key={discipline.slug} style={{ marginBottom: dIdx < TREATMENT_LIST.length - 1 ? 96 : 0 }}>
                <Reveal>
                  <div style={{
                    paddingBottom: 24, borderBottom: "1px solid var(--ink-20)", marginBottom: 32,
                  }}>
                    <h2 style={{
                ...
                {subs.map(([subSlug, subTitle]) => {
                  ...
                  return (
                    <div key={subSlug} style={{ marginBottom: 48 }}>
                      <Reveal delay={60}>
                        <div style={{
                          padding: "24px 0 12px", borderBottom: "1px solid var(--ink-20)",
                        }}>
                          <h3 style={{
                      ...
                      <div>
                        {sub.treatments.map((t, tIdx) => {
                          ...
                          const isIncluded = t.priceFromAud === "included" || t.priceFromAud === "complimentary";
                          return (
                            <Reveal key={tIdx} delay={tIdx * 20}>
                              <a href={`treatment-${subSlug}.html`} style={{
                                display: "grid", gridTemplateColumns: "1.1fr 1.6fr 220px 36px",
                                gap: 28, padding: "20px 0", borderBottom: "1px solid var(--ink-20)",
                              ...
                              >
                                ...
                                <p style={{ margin: 0, fontFamily: "var(--font-serif)", fontStyle: "italic", fontSize: 15, color: "var(--ink-60)", lineHeight: 1.5 }}>
                                  {t.short}
                                </p>
                                <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 3 }}>
                                  {isIncluded ? (
                                    <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "0.18em", color: "var(--accent-deep)", textTransform: "uppercase" }}>
                                      Included
                                    </span>
                                  ) : isNumber ? (
                                    <>
                                      <span style={{
                                        ...
                                        letterSpacing: "0.18em", color: "var(--accent-deep)",
                                        whiteSpace: "nowrap",
                                      }}>{fmtIDR(t.priceFromAud)}</span>
                                      <span style={{ fontFamily: "var(--font-serif)", fontStyle: "italic", fontSize: 13, color: "var(--ink-60)" }}>
                                        ≈ AUD {t.priceFromAud.toLocaleString("en-AU")}
                                      </span>
                                    </>
                                  ) : (
                                    <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "0.18em", color: "var(--ink-60)" }}>
                                      On request
                                    </span>
                                  )}
                                </div>
                                <span style={{ textAlign: "right", color: "var(--accent-deep)", fontFamily: "var(--font-serif)", fontSize: 22 }}>→</span>
                              </a>
                            </Reveal>
                          );
                        })}
      ...
      <section className="page-section" style={{ paddingTop: 0 }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "32px 0", borderTop: "1px solid var(--ink-20)" }}>
          <p style={{
            margin: 0, fontFamily: "var(--font-serif)", fontStyle: "italic",
            fontSize: 14, color: "var(--ink-60)", textAlign: "center", lineHeight: 1.55,
          }}>
            Prices indicative for international patients. AUD shown at 1 AUD ≈ Rp 10,500 (May 2026).
            Final quotes are tailored after consultation. Recovery stays, transfers, and twelve months of
            telehealth follow-up included on most surgical packages.
          </p>
        </div>
      </section>
