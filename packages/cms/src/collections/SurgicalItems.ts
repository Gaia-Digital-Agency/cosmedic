...
export const SurgicalItems: CollectionConfig = {
  slug: 'surgical-items',
  labels: { singular: 'Surgical', plural: 'Surgical' },
  admin: {
    useAsTitle: 'name',
    defaultColumns: ['name', 'mainCategory', 'subCategory', 'sortOrder'],
    group: 'Pricing',
    description: 'Surgical procedure price rows rendered in the Surgical table on /pricing. Grouped by mainCategory (Arm · Body · Breast · Eyelid · Face & Neck · Rhinoplasty · Others).',
  },
  access: {
    read: readPublic,
  ...
  fields: [
    ...
    { name: 'catalogueGroup', type: 'text', defaultValue: 'surgical',
      admin: { hidden: true, description: 'Hardcoded to "surgical" — do not change.' } },
    { name: 'mainCategory', type: 'text',
      admin: { description: 'Main category — e.g. "Body", "Breast", "Face & Neck".' } },
    { name: 'subCategory', type: 'text',
      admin: { description: 'Optional 3rd-level grouping label inside Main Category.' } },
    { name: 'unit', type: 'text',
      admin: { description: 'e.g. "per procedure", "per area".' } },
    {
      name: 'audienceTier',
      type: 'select',
      ...
      admin: { description: 'Pricing tier for multi-tier rows.' },
    },
      ...
      admin: { description: 'Optional hero image.' } },
    {
      ...
      fields: [
        ...
        { name: 'priceNotes', type: 'text', admin: { description: 'e.g. "Local Anaesthesia".' } },
