      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 24 }}>
        {procs.map((p, i) => (
          <Reveal key={p.id} delay={i * 40}>
            <a href={`/procedures/${p.slug}`} style={{ display: 'block', padding: 20, border: '1px solid var(--ink-20)', color: 'inherit', textDecoration: 'none' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${block.layout === 'grid' ? 4 : 4}, minmax(0, 1fr))`, gap: 20 }}>
        {surgeons.map((s, i) => (
          <Reveal key={s.id} delay={i * 30}>
            <a href={`/experts/${s.slug}`} style={{ color: 'inherit', textDecoration: 'none', display: 'block' }}>
