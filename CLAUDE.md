...
| [docs/remap_plan.md](docs/remap_plan.md) | **Phase R implementation plan.** R0 = top-level Bucket realignment (24-file admin.group sweep). R1 = Contact detail. R2 = Homepage detail. R7 = Journey detail. R8 = About detail. R3/R4/R5/R6 = per-Bucket detail, pending map sign-off. Rules 8 (universal coverage, no duplication) + 9 (full Payload capability preserved) added. |
...
## Current state (Phase R0 shipped 2026-05-24 · Phase Q 18/19 · Phase 8 live · q17 deferred)
...
- `packages/cms` — Payload 3.84.1 on Next.js 15.4.11 + Postgres adapter, port **4007**. Admin white-labelled as **Cosmedic CMS** (Cormorant Garamond + JetBrains Mono, brand-beige palette from `docs/assets/brand-guidelines.pdf`). Light theme default (q19, 2026-05-24).
  - **17 collections** in `src/collections/` (as of 2026-05-24 — `PricingTiers` removed in q5 `a1601e5`; `InclusionItems` + `ExclusionItems` removed in q19 `1b35bfb`; orphan `Pages` collection removed (Rule 4 gate resolved 2026-05-24 — DB table `pages` retained with 8 backup rows)): Users · Media · Surgeons · Disciplines · SubCategories · Procedures · PriceListItems · InjectableProducts · MachineTreatments · HairRemovalAreas · BeforeAfterCases · Stories · PressMentions · Awards · RecoveryStays · BlogPosts · BlogTags · Authors · JourneySteps · Enquiries.
  ...
  - **Admin taxonomy (R0 shipped 2026-05-24 `97c1e23`):** 9 prefixed Buckets in code matching site IA reading order — **a. Homepage** (8) · **b. Treatments** (4) · **c. Doctors** (2) · **d. Results** (5, absorbed `/stories`) · **e. Pricing** (2) · **f. Journey** (4, absorbs `/recovery-stays`) · **g. Contact** (5) · **h. About** (8, absorbed `/blog` + `/press` + `/privacy`) · **i. Media Library** (1) · + Users ungrouped. Pure `admin.group` string sweep across 39 files; no field changes, no data migration, no UI change. Per-Bucket detail phases (R1–R8) planned in [docs/remap.md](docs/remap.md) + [docs/remap_plan.md](docs/remap_plan.md), gated on user sign-off per phase. **Execution order**: R0 ✅ → R8 → R7 → R1 → R4 → R6 → R5 → R3 → R2 (lowest-risk-first).
...
### Item 2 Step 10 — Rule 4 gate, ✅ resolved 2026-05-24

`Pages` collection unregistered from `payload.config.ts` and `src/collections/Pages.ts` removed (verified: `/api/pages` → 404). DB table `public.pages` retained with its 8 rows as a one-shot rollback backup — drop manually if/when sign-off is final.
