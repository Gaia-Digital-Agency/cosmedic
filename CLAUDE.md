...
- `packages/cms` — Payload 3.84.1 on Next.js 15.4.11 + Postgres adapter, port **4007**. Admin white-labelled as **Cosmedic CMS** (Cormorant Garamond + JetBrains Mono, brand-beige palette from `docs/assets/brand-guidelines.pdf`). Light theme default (q19, 2026-05-24).
  ...
  - **Admin taxonomy (current state, drift from doc spec):** 8 buckets in code — Homepage · Treatments · Doctors · Results · Pricing · Journey · Contact · Media Library (+ Users ungrouped). The "Blog" bucket mentioned in the original spec never materialised (blog items currently sit in Journey). **Phase R remap planned** ([docs/remap.md](docs/remap.md), [docs/remap_plan.md](docs/remap_plan.md)) — target = 9 prefixed buckets matching site IA: a. Homepage · b. Treatments · c. Doctors · d. Results (absorbs `/stories`) · e. Pricing · f. Journey (absorbs `/recovery-stays`) · g. Contact · h. About (footer column — `/blog` + `/press` + `/privacy`) · i. Media Library · + Users ungrouped. Phase R0 (24-file `admin.group` sweep) ungated; per-Bucket detail phases gated on per-map user sign-off.
