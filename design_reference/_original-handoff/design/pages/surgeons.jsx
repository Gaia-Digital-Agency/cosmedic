...
const SurgeonCard = ({ s, idx }) => (
  <Reveal key={s.slug} delay={idx * 60} y={20}>
    <a href={`surgeon-${s.slug}.html`} style={{ color: "inherit", display: "block" }}>
      <div className="surgeon-card" data-surgeon={s.slug}>
        <div className="surgeon-card-img" style={{ aspectRatio: "4 / 5" }}>
          <Img src={s.img} fallbackLabel={`DR. ${s.common.toUpperCase()}`} fallbackHue={s.hue} alt="" />
        </div>
        <div className="surgeon-card-meta" style={{ paddingTop: 20 }}>
          <Mono>{(idx + 1).toString().padStart(2, "0")}</Mono>
          <h4 style={{ fontSize: 22, marginTop: 6 }}>{s.title} {s.name}</h4>
          {s.suffix && <span className="surgeon-train" style={{ fontSize: 14, marginTop: 2 }}>{s.suffix}</span>}
          <span className="surgeon-train" style={{ fontSize: 14, marginTop: 6, color: "var(--ink-80)" }}>{s.spec}</span>
          <span className="mono" style={{ marginTop: 12, color: "var(--accent-deep)" }}>{s.proc}</span>
        </div>
      </div>
    </a>
  </Reveal>
...
const Page = () => (
  <PageShell activePage="surgeons">
    <ChapterOpener
      ...
      breadcrumbs={[{ label: "BIMC CosMedic", href: "index.html" }, { label: "Surgeons" }]}
    />

    {/* Lead surgeon — featured */}
    <section className="page-section">
      <Reveal><Eyebrow>Lead Plastic Surgeon</Eyebrow></Reveal>
      <div className="surgeons-feature" style={{ marginTop: 40, paddingBottom: 0, borderBottom: "none" }}>
        <Reveal delay={120}>
    ...
    <section className="page-section tinted">
      <Reveal>
        <div className="section-head">
          ...
          <div>
            <h2 className="section-title"><span className="italic">Reconstructive</span> & aesthetic.</h2>
            <p className="section-lede">Our plastic surgery team holds combined fellowships from Korea, Japan, Singapore, and Indonesia — and ISAPS, FICS, and craniomaxillofacial subspecialty credentials.</p>
          </div>
        </div>
      </Reveal>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 32 }}>
        {PLASTIC.slice(1).map((s, i) => <SurgeonCard key={s.slug} s={s} idx={i + 1} />)}
      </div>
    </section>
    ...
    <section className="page-section">
      <Reveal>
        <div className="section-head">
          ...
          <div>
            <h2 className="section-title"><span className="italic">Quiet</span> non-surgical.</h2>
            <p className="section-lede">Anti-aging, dermatology, venereology and aesthetics — for the considered, everyday refinements between bigger decisions.</p>
          </div>
        </div>
      </Reveal>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 24 }}>
        {AESTHETIC.map((s, i) => <SurgeonCard key={s.slug} s={s} idx={PLASTIC.length + i} />)}
      </div>
    </section>
  </PageShell>
);
